...
export default function ValueSetManagement({ userAgency }: { userAgency: string }) {
  ...
  const [viewingItem, setViewingItem] = useState<ValueSet | null>(null);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {isProvince ? (
          <button onClick={() => { setEditingItem(null); setFormData({ valueSetId: '', valueSetName: '', entityId: '', sourceCode: '', term: '', valueSource: '' }); setShowModal(true); }} className="flex items-center gap-2 px-4 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700"><Plus className="w-4 h-4" />新增值集</button>
        ) : (
          <div className="rounded-lg bg-amber-50 px-4 py-2 text-sm text-amber-700">地市账号仅可查看值集字典，不可新增、编辑或删除</div>
        )}
      </div>
