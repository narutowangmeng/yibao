...
export default function DataModelManagement({ userAgency }: { userAgency: string }) {
  ...
  const [viewingItem, setViewingItem] = useState<DataElement | null>(null);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {isProvince ? (
          <button onClick={() => { setEditingItem(null); setFormData({ dmId: '', deId: '', deName: '', dataType: '字符型', deNote: '' }); setShowModal(true); }} className="flex items-center gap-2 px-4 py-2 bg-cyan-600 text-white rounded-lg">
            <Plus className="w-4 h-4" />
            新增数据元素
          </button>
        ) : (
          <div className="rounded-lg bg-amber-50 px-4 py-2 text-sm text-amber-700">地市账号仅可查看全省数据模型，不可新增、编辑或删除</div>
        )}
      </div>
              ...
              <tr key={item.id} className="border-t border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    <button onClick={() => setViewingItem(item)} className="p-1.5 text-gray-500 hover:text-cyan-600"><Eye className="w-4 h-4" /></button>
                    {isProvince && (
                      <>
                        <button onClick={() => { setEditingItem(item); setFormData({ dmId: item.dmId, deId: item.deId, deName: item.deName, dataType: item.dataType, deNote: item.deNote }); setShowModal(true); }} className="p-1.5 text-gray-500 hover:text-cyan-600"><Edit2 className="w-4 h-4" /></button>
                        <button onClick={() => setElements(elements.filter((e) => e.id !== item.id))} className="p-1.5 text-gray-500 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                      </>
                    )}
                  </div>
                </td>
