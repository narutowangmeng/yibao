...
interface ReimbursementLedgerItem {
  id: string;
  personName: string;
  idCard: string;
...
const initialData: ReimbursementLedgerItem[] = [
  ...
  { id: 'BX320800202604008', personName: '朱雨彤', idCard: '320802199912167245', reimbursementType: '住院报销', agency: '淮安', hospital: '淮安市第一人民医院', hospitalLevel: '三级甲等', visitDate: '2026-04-15', totalAmount: 9620, inScopeAmount: 8840, selfPayAmount: 2540, reimbursementAmount: 6300, status: '已办结', diagnosis: '子宫肌瘤', settlementNo: 'JSJS20260415008', auditOpinion: '病案首页及清单齐全', reviewerName: '严峰', settleDate: '2026-04-18' },
  ...
  { id: 'BX321000202604010', personName: '郭天宇', idCard: '321002198610024671', reimbursementType: '特殊药品报销', agency: '扬州', hospital: '扬州大学附属医院', hospitalLevel: '三级甲等', visitDate: '2026-04-17', totalAmount: 5680, inScopeAmount: 5360, selfPayAmount: 1520, reimbursementAmount: 3840, status: '已办结', diagnosis: '强直性脊柱炎', settlementNo: 'JSJS20260417010', auditOpinion: '双通道药品按比例支付', reviewerName: '邹琳', settleDate: '2026-04-20' },
  ...
  { id: 'BX320900202604019', personName: '郑博文', idCard: '320921198612302416', reimbursementType: '住院报销', agency: '盐城', hospital: '盐城市第一人民医院', hospitalLevel: '三级甲等', visitDate: '2026-04-22', totalAmount: 16780, inScopeAmount: 15260, selfPayAmount: 4680, reimbursementAmount: 10580, status: '退回补件', diagnosis: '脑梗死恢复期', settlementNo: 'JSJS20260422019', auditOpinion: '康复项目频次需复核', reviewerName: '曹颖', settleDate: '' },
  { id: 'BX321000202604020', personName: '陶诗雅', idCard: '321003199611058260', reimbursementType: '特殊门诊', agency: '扬州', hospital: '扬州市妇幼保健院', hospitalLevel: '三级甲等', visitDate: '2026-04-18', totalAmount: 2480, inScopeAmount: 2280, selfPayAmount: 620, reimbursementAmount: 1660, status: '已办结', diagnosis: '甲状腺功能减退', settlementNo: 'JSJS20260418020', auditOpinion: '用药周期和剂量合理', reviewerName: '邹琳', settleDate: '2026-04-21' },
];

export default function ReimbursementQueryCenter({ onBack }: { onBack: () => void }) {
  const [keyword, setKeyword] = useState('');
  const [rows, setRows] = useState<ReimbursementLedgerItem[]>(initialData);
  const [selectedItem, setSelectedItem] = useState<ReimbursementLedgerItem | null>(null);
  const importRef = useRef<HTMLInputElement>(null);

  const filteredRows = useMemo(
    () => rows.filter((item) => [item.id, item.personName, item.idCard, item.hospital, item.settlementNo].some((value) => value.includes(keyword))),
    [rows, keyword],
  );
  ...
  const downloadTemplate = () => {
    writeWorkbook(initialData.slice(0, 3).map(toExportRow), '费用报销导入模板.xlsx');
  };
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">报销申请、审核进度、结算结果统一查询与导入导出</p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 grid grid-cols-4 gap-4">
          ...
          <div className="rounded-2xl border border-cyan-200 bg-cyan-50 p-4">
            <div className="text-sm text-cyan-700">涉及医院</div>
            <div className="mt-2 text-3xl font-bold text-cyan-600">{new Set(rows.map((item) => item.hospital)).size}</div>
          </div>
        </div>
